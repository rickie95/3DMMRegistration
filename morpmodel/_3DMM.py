...
class _3DMM:
    ...
    def getProjectedVertex(self,vertex,S,R,t):
        ...
        app = np.tile(t, (1,rotPc.shape[0]))
        return S.dot(np.transpose(rotPc)) + app
    ...
    def alphaEstimation(self, landImage, projLandModel, Components_res, id_landmarks_3D, S, R, t, Weights, _lambda):
        ...
        if _lambda == 0:
            ...
            with np.errstate(divide='ignore'):
                invW = np.diag( _lambda/(np.diagflat(Weights))) # as diag in malab
            #if invW.shape[1] != 1:
             #   invW = np.diagflat(invW)
    ...
    def alpha_estimation_test(self, avg_colors, target_colors, Components,  Weights, _lambda):
        avg_colors = np.reshape(avg_colors, (avg_colors.shape[0], 1))
        target_colors = np.reshape(target_colors, (target_colors.shape[0], 1))

        X = target_colors - avg_colors
        Y = Components
        with np.errstate(divide='ignore'):
            invW = np.diag(_lambda / (np.diagflat(Weights)))  # as diag in malab
        var = (np.transpose(Y)).dot(Y)
    ...
    def HPR(self, p, C, param):
        ...
        #into_unique = ConvexHull(vect_conv_hull)
